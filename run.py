...
class VideoDataset(Dataset):
    ...
    def __init__(self, root_dir, dataset_choice="train", nb_frames=10):
        ...
        if  self.dataset_choice == "train":
            ...
            #self.root_dir =  "/raid/home/automathon_2024/account21/Desktop/automathon-2024/train_crops"
        elif  self.dataset_choice == "test":
            self.root_dir = os.path.join(root_dir, "test_dataset")
            #self.root_dir =  "/raid/home/automathon_2024/account21/Desktop/automathon-2024/test_crops"
    ...
    def __getitem__(self, idx):
        ...
        if self.dataset_choice == "test":
            return video[0], ID  # frame-by-frame
        else:
            label = self.data[self.video_files[idx]]
            return video[0], label, ID  # frame-by-frame
...
class DeepfakeDetector(nn.Module):
    def __init__(self, nb_frames=10):
        super().__init__()
        # Using the pre-trained EfficientNet model
        self.model = timm.create_model('efficientnet_b0', pretrained=True)

        self.model.conv_stem = nn.Conv2d(3,32, kernel_size=(3, 3), stride=(2, 2), padding=(1, 1), bias=False)
        # Freezing the parameters of the model (except the modified layer)
        for p in self.model.parameters():
            p.requires_grad = False
            
        self.fc = nn.Sequential(
            nn.Linear(1000, 512),  # Adjust input size to match the output of EfficientNet
            nn.ReLU(),
            nn.Dropout(0.5),
    ...
    def forward(self, x):
        x = x.view(x.size(0), -1, x.size(-2), x.size(-1))  # Flattening for 2D convolution
        features = self.model(x)
        output = self.fc(features.view(features.size(0), -1))  # Flatten features for linear layer
        return output
...
#optimizer = torch.optim.Adam(model.parameters(), lr=0.01,weight_decay=0.001)# Adding weight decay for regularization
...
with open("submission_tramonte.csv", "w") as file:
